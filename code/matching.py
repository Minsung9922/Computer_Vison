...
class TrafficWeak(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('배개찾기')
        self.setGeometry(200, 200, 700, 200)

        signButton = QPushButton('배개 등록', self)
        roadButton = QPushButton('책상 불러옴', self)
        recognitionButton = QPushButton('인식', self)
        quitButton = QPushButton('나가기', self)
        self.label = QLabel('환영!', self)
        ...
        self.signFiles = [['pillow.jpg', '배개']]  # 배개 모델 영상
        self.signImgs = []  # 배개 모델 영상 저장

    def signFunction(self):
        self.label.clear()
        self.label.setText('찾을 배개를 등록합니다.')
    ...
    def roadFunction(self):
        if self.signImgs == []:
            self.label.setText('먼저 배개을 등록하세요.')
        else:
    ...
    def recognitionFunction(self):
        if self.roadImg is None:
            self.label.setText('먼저 책상 영상을 입력하세요.')
        else:
            sift = cv.SIFT_create()

            KD = []  # 여러 배개 영상의 키포인트와 기술자 저장
            ...
            if len(GM[best]) < 4:  # 최선의 배개 매칭 쌍 4개 미만이면 실패
                self.label.setText('배개가 없습니다.')
                ...
                h1, w1 = self.signImgs[best].shape[0], self.signImgs[best].shape[1]  # 배개 영상의 크기
                h2, w2 = self.roadImg.shape[0], self.roadImg.shape[1]  # 책상 영상의 크기
                ...
                self.label.setText(self.signFiles[best][1] + '푹주무세요')
